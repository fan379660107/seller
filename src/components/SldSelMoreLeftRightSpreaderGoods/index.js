              ...
              <Scrollbars
                ...
                <div className={`${styles.right} ${global.flex_row_start_start}`} style={{ height: height }}>
                  ...
                  selectedRows.map((item, index) => {
                    return <div key={index} className={`${styles.right_item_wrap} ${global.flex_column_start_start}`}
                                style={{ marginBottom: index == selectedRows.length - 1 ? 10 : 0 }}>
                      <div  className={`${styles.item} ${global.flex_row_start_start}`}
                      ...
                      <div className={`${styles.spreader_goods_info} ${global.flex_row_between_center}`}>
                        <div className={`${styles.spreader_goods_info_left} ${global.flex_row_start_center}`}>
                          ...
                          <InputNumber value={item.commission!=undefined&&item.commission?item.commission:0.01} placeholder={'佣金'} min={0.01} max={item.goodsPrice*1-0.01} precision={2}
                        ...
                        <div className={`${styles.spreader_goods_info_left} ${styles.spreader_goods_info_right} ${global.flex_row_start_center}`}>
                          ...
                          <Select value={item.labelId!=undefined&&item.labelId?item.labelId:label_data[0].labelId}
                                  placeholder={'标签'}
                                  onChange={(e) => this.handleLabel(e,item.goodsId)}
                                  getPopupContainer={triggerNode => triggerNode.parentNode}
                                  >
                            {label_data.map((item, index) => {
                              return <Select.Option key={index} value={item.labelId}>{item.labelName}</Select.Option>;
                            })}
                          </Select>
                        </div>
                      </div>
