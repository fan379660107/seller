...
import diy from '../../pages/pcdecorate/diy_page.less';
		...
		for (let i in data) {
			if (data[i].key == key) {
				...
				if (com_type == 'inputnum') {
					...
					tmp_val = e;//moment {sldComLanguage('对象')}
				} else {
		...
		return <Fragment>
			...
			>
				...
				<div className={global.common_bottom_wrap}>
					<div className={`${diy.save_diy_page} ${diy.allow_save}`}
						 onClick={(e) => this.sldConfirm(e)}>
						{sldComLanguage('保 存')}
					</div>
				</div>
			</Form>
