...
export default class SldSelMoreLeftRight extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      search_data: [{
        type: 'input',
        label: `${sldComLanguage('视频名称')}`,
        name: 'video_name',
        placeholder: `${sldComLanguage('请输入短视频名称')}`,//请输入短视频名称
      }, {
        type: 'select',
        label: `${sldComLanguage('视频标签')}`,
        name: 'label_id',
        placeholder: `${sldComLanguage('请选择视频标签')}`,
        sel_data: [],
        diy: true,
        sele_key: 'label_id',
  ...
  sldConfirm = () => {
    ...
    if (selectedRowKeys.length > 0) {
      if (this.props.extra.min_num != undefined && this.props.extra.min_num > 0 && selectedRowKeys.length < this.props.extra.min_num) {
        failTip(`${sldComLanguage('该模块至少需要选择')}${this.props.extra.min_num}${sldComLanguage('个短视频')}`);
        return false;
      }
      ...
      if (this.props.extra.max_num != undefined && this.props.extra.max_num > 0 && selectedRowKeys.length > this.props.extra.max_num) {
        failTip(`${sldComLanguage('该模块最多选择')}${this.props.extra.max_num}${sldComLanguage('个短视频')}`);
        return false;
      }
                    ...
                    return <div key={index} className={`${styles.item} ${global.flex_row_start_start}`}
                      ...
                      <div className={`${styles.item_left} ${global.flex_row_center_center}`}>
                        ...
                        <div className={`${styles.play_num}`}>
                          <span>{sldComLanguage('播放量:')}{formatNumW(item.click_num)}</span>
                        </div>
                      </div>
