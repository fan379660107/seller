...
export default class MaxHeightScroll extends PureComponent {
  ...
  commonCon = (val, index) => {
    ...
    if (val.type == 'Tree') {
      //数形组件
      return (
        <FormItem key={index} extra={val.extra} style={{ width: ' 80%' }}>
          {getFieldDecorator(val.name, {
            initialValue: val.initialValue == '' ? undefined : val.initialValue,
            rules: val.rules,
          })(
            <Tree defaultExpandAll={true} checkable onCheck={val.onCheck}>
              {val.data.map(item => {
                return (
                  <TreeNode title={item.title} key={item.key}>
                    {item.children != undefined &&
                      ...
                      item.children.map(items => {
                        return <TreeNode title={items.title} key={items.key} />;
                      })}
                  </TreeNode>
                );
              })}
            ...
            data.map((val, index) => {
              return (
                ...
                >
                  ...
                  >
                    <span className={styles.sld_det_r_text} style={{ fontWeight: '600' }}>
                  ...
                  >
                    <Scrollbars autoHeight autoHeightMin={50} autoHeightMax={max_height - 30}>
