    ...
    this.state = {
      ...
      column: [
        ...
        },
        {
          ...
          render: (text, record) => {
            return (
              <Popconfirm placement={'rightBottom'} title={`${sldComLanguage('确定删除该行吗？')}`}
                          onConfirm={() => this.remove(record.key)} okText={`${sldComLanguage('确定')}`} cancelText={`${sldComLanguage('取消')}`}>
                <a style={{
                  marginTop: 5,
