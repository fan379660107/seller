...
export default class SearchAttr extends Component {
    ...
    this.state = {
      ...
      columns: [
        ...
        },
        {
          ...
          width: 80,
          render: (text, record) => (
            <Switch checkedChildren={`${sldComLanguage('启用')}`}
                    onChange={(checked) => this.operateAttr(record.parameterId, checked ? 'start' : 'stop')}
  ...
  editAttr = (val) => {
    ...
    for (let i in operateData) {
      if(operateData[i].name == 'attribute'){
        operateData[i].tags = val.parameterValue.split(',');
      }else{
  ...
  get_list = (params) => {
    ...
    dispatch({
      ...
      payload: {...params,groupId:this.state.query.id},
      callback: (res) => {
  ...
  sldHandleConfirm = (val) => {
    ...
    if(attr_data.tags.length == 0){
      ...
      val.parameterValue = attr_data.tags.join(',');
    }
    this.setState({ submiting: true });
    if (type == 'edit') {
    ...
    return (
      <div className={global.common_page} style={{ flex: 1 }}>
        ...
        <SldModal
          ...
          submiting={submiting}
          maxLengthLimit={20}
          width={500} Promotion_flash_sale
