...
export default class BillDetail extends Component {
      ...
      columns_order: [
        ...
        },
        {
          title: <div style={{ position: 'relative' }}>
            {sldComLanguage('佣金(元)')}
            <Tooltip placement="bottomLeft" title={`${sldComLanguage('平台抽取的分类佣金')}`}>
        ...
        {
          title: <div style={{ position: 'relative' }}>
            {sldComLanguage('退还佣金(元)')}
            <Tooltip placement="bottomLeft" title={`${sldComLanguage('订单发生退款退货时，平台将不抽取这部分佣金')}`}>
              <div style={{ right: -15, top: 2, position: 'absolute' }}>{sldSvgIcon('#bfbbba', 14, 14, 'wen')}</div>
            </Tooltip>
          </div>,
  ...
  render() {
    const { bill_base_data, bill_detail_data, bill_progress_data, bill_settle_img, preview_img, show_preview_modal, spreaderFlag, columns_order } = this.state;
    return (
