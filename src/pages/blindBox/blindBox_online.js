...
@Form.create()
export default class BlindBoxOnline extends Component {
  constructor(props) {
    super(props);
    this.state = {
      search_height:0,
                ...
                ?<Fragment>
                  {text.map((item,index)=>{
                    return <div key={index}>
                      {quillEscapeToHtml(quillEscapeToHtml(item))}
                    </div>
                  })}
                </Fragment>
                :'--'
        ...
        <div className={global.operate_bg}
          ...
          <div style={{ display: 'flex', flexDirection: 'row' }}>
            ...
            {selectedRowKeys.length == 0 ? sldIconBtn(() => {
              ...
              //确认下架选中的商品吗？
            }, `${sldComLanguage('下架')}`, 7, 0, 13, 13, 3, 'ziyuan31', '#f9a006') : sldPopConfirm('leftBottom', `${sldComLanguage('确认下架选中的商品吗？')}`, () => this.operateGoods({ goodsIds: selectedRowKeys.join(',') }, 'lockup'), `${sldComLanguage('确定')}`, `${sldComLanguage('取消')}`, sldIconBtn(null, `${sldComLanguage('下架')}`, 7, 0, 13, 13, 3, 'ziyuan31', '#f9a006'), 0, 0, '#f9a006')}
