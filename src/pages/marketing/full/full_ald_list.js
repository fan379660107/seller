...
@Form.create()
export default class FullAldList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      operateData: [],
      ...
      columns: [
        ...
        },
        {
          ...
          render: (text, record) => (
            <Fragment>
              <Link to={{
                pathname: '/marketing/full_ald_to_add',
                query: {
                  id: record.fullId,
                  tar:'view'
                },
  ...
  operate = (id, type) => {
    ...
    let dis_type = '';
    if (type == 'invalid') {
      dis_type = 'promotion/invalid_full_ald';
      param_data.fullId = id;
    } else if (type == 'copy') {
      dis_type = 'promotion/copy_full_ald';
      param_data.fullId = id;
    } else if (type == 'del') {
      dis_type = 'promotion/del_full_ald';
      param_data.fullId = id;
    } else if (type == 'publish') {
      dis_type = 'promotion/publish_full_ald';
      param_data.fullId = id;
    }
  ...
  get_list = (params) => {
    ...
    const { dispatch } = this.props;
    dispatch({
      type: 'promotion/get_full_ald_list',
      payload: { ...params },
      callback: (res) => {
  ...
  render() {
    ...
    return (
      <div className={global.common_page} style={{ flex: 1, paddingTop: 0 }}>
        {getSldEmptyH(10)}
        {sldLlineRtextAddGoodsAddMargin('#FA6F1E', `${sldComLanguage('满N元折扣活动')}`, 0, 0, 10)}
        ...
        <div className={global.operate_bg}>
          <Link to={{
            ...
            {sldIconBtn(() => null, `${sldComLanguage('新建满N元折扣活动')}`, 7, 0, 12, 12, 3, 'fabu1', '#08A9B7')}
          </Link>
        </div>
        {/*公共功能条-end*/}
