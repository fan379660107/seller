...
@Form.create()
export default class AddFullAld extends Component {
  constructor(props) {
    super(props);
    sthis = this;
    this.state = {
  ...
  get_detail = async (id) => {
    ...
    let _this = this;
    dispatch({
      type: 'promotion/get_full_ald_detail',
      payload: { fullId: id },
      callback: async (res) => {
        if (res.state == 200) {
          detail = res.data;
...
//保存并新增事件
  handleSaveAllData = () => {
    const { dispatch } = this.props;
    const { query, ladder_promotion,minusValueMap } = this.state;
    let param = {};
    this.props.form.validateFieldsAndScroll((err, values) => {
        if (!err) {
          ...
          if (query.id != undefined && query.id > 0 && query.tar == 'edit') {
            ...
            dis_type = 'promotion/edit_full_ald';
          } else {
            //新增满N元折扣
            dis_type = 'promotion/add_full_ald';
          }
          dispatch({
            type: dis_type,
            payload: param,
  ...
  render() {
    ...
    return (
      <div className={`${promotion.full_activity} ${global.common_page} ${global.com_flex_column}`}
        ...
        {sldLlineRtextAddGoodsAddMargin('#69A2F2', `${sldComLanguage('满N元折扣活动')}`, 0, 0, 10)}
        {getSldHorLine(1)}
                      ...
                      return <Fragment key={index}>
                          ...
                          <div className={`${promotion.right}`}>
                            ...
                            >
                              {getFieldDecorator(`fullValue_${item.key}`, {
                                ...
                                <InputNumber disabled={viewFlag} style={{ width: 150 }} min={1} max={9999999} precision={2}/>,
                              )}
                            </FormItem>
                          </div>
                        </div>
                        ...
                        <div className={`${promotion.item} ${global.flex_row_start_start}`}>
                          ...
                          <div className={`${promotion.right} ${global.flex_column_start_start}`}>
                            ...
                            >
                          <span
                            style={{ marginRight: 10 }}>{sldComLanguage('打')}</span>{getFieldDecorator(`minusValue_${item.key}`, { initialValue: item.minusValue })(
