...
@Form.create()
export default class FullAsmList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      operateData: [],
      ...
      columns: [
        ...
        },
        {
          ...
          render: (text, record) => (
            <Fragment>
              <Link to={{
                pathname: '/marketing/full_asm_to_add',
                query: {
                  id: record.fullId,
                  tar:'view'
                },
  ...
  //阶梯满减操作  type: invalid 失效 copy 复制  del 删除
  operate = (id, type) => {
    ...
    let dis_type = '';
    if (type == 'invalid') {
      dis_type = 'promotion/invalid_full_asm';
      param_data.fullId = id;
    } else if (type == 'copy') {
      dis_type = 'promotion/copy_full_asm';
      param_data.fullId = id;
    } else if (type == 'del') {
      dis_type = 'promotion/del_full_asm';
      param_data.fullId = id;
    } else if (type == 'publish') {
      dis_type = 'promotion/publish_full_asm';
      param_data.fullId = id;
    }
  ...
  get_list = (params) => {
    ...
    const { dispatch } = this.props;
    dispatch({
      type: 'promotion/get_full_asm_list',
      payload: { ...params },
      callback: (res) => {
  ...
  render() {
    const { selectedRows, selectedRowKeys, search_data, columns, initLoading, data, show_preview_modal, preview_img, preview_alt_con, view_spec_data, modalVisible, title, modal_width, operateData, show_foot, submiting } = this.state;
    return (
      <div className={global.common_page} style={{ flex: 1, paddingTop: 0 }}>
        {getSldEmptyH(10)}
        {sldLlineRtextAddGoodsAddMargin('#FA6F1E', `${sldComLanguage('阶梯满减活动')}`, 0, 0, 10)}
        ...
        <div className={global.operate_bg}>
          <Link to={{
            ...
            {sldIconBtn(() => null, `${sldComLanguage('新建阶梯满减活动')}`, 7, 0, 12, 12, 3, 'fabu1', '#08A9B7')}
          </Link>
        </div>
        {/*公共功能条-end*/}
