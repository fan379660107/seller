...
@Form.create()
export default class JoinedGoodsLists extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (text, record) => (
            <Fragment>
              <Link to={{
                pathname: '/marketing/spell_group_team_list',
                query: {
                  spellId: props.location.query.id,
  ...
  operate = (record, type) => {
    ...
    param_data.spellId = query.id
    this.setState({ submiting: true });
    dispatch({
      type: 'promotion/del_spell_group_goods',
      payload: param_data,
      callback: (res) => {
        if (res.state == 200) {
          sucTip(res.msg);
  ...
  get_list = (params) => {
    ...
    dispatch({
      type: 'promotion/get_spell_group_joined_goods_lists',
      payload: { ...params, spellId: query.id },
      callback: (res) => {
