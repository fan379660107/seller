                      ...
                      <div className={`${order.order_info_num} ${global.flex_row_between_center}`}>
                        <div className={`${order.left} ${global.flex_row_start_center}`}>
                          <div className={`${order.left_activity_info} ${global.flex_row_start_center}`}>
                            <img className={`${order.activity_icon}`}
                                 src={require('@/assets/sld_spell_group_order_item_icon.png')}/>
                            <span className={`${order.num} ${order.activity_name}`}
                                  title={item.spellName}>{item.spellName}</span>
                          </div>
                          ...
                          <div className={order.spell_state}><span
                            className={order.spell_state_val}>{item.finishType == 2 &&
                        ...
                        <div className={`${order.right} ${global.flex_row_end_center}`}>
                          <span
                            className={order.spell_time}>{sldComLanguage('开团时间')}：{item.createTime} ～ {item.endTime}</span>
                        </div>
                      </div>

                      <div className={`${global.flex_row_start_start}`}
                           style={{ width: '100%', alignItems: 'stretch' }}>
                        <div className={`${order.spell_group_goods} ${global.flex_column_center_center}`}
                        ...
                        <div className={`${global.flex_column_start_start}`} style={{ width: '85%' }}>
                          {item.memberList && item.memberList.length > 0 && item.memberList.map((child, child_index) => {
                            return <div key={child_index}
                              ...
                              <div
                                ...
                                <div className={`${order.goods_item} ${global.flex_row_start_center}`}
                                     style={{ width: '100%' }}>
                                  ...
                                  <div className={`${global.flex_column_between_start}`}
                                    ...
                                    <div className={`${global.flex_row_start_center}`}>
                                      <div className={`${order.goods_img_wrap} ${global.flex_row_center_center}`}>
                                        <div className={order.member_avatar}
                                      ...
                                      <div className={`${order.goods_info} ${global.flex_column_start_start}`}>
                                        ...
                                        <span className={`${order.goods_spec}`}
                                              title={child.specValues}>{child.specValues}</span>
                                      </div>
                                    </div>

                                    <div className={`${order.member_order_info}`}>
                                      <span className={order.tip}>{sldComLanguage('订单号：')}<span
                                        className={order.content}>{child.orderSn}</span></span>
                                      <span className={order.tip}
                                            style={{ marginLeft: 25 }}>{sldComLanguage('下单时间：')}<span
                                        className={order.content}>{child.createTime}</span></span>
                                    </div>

                                  </div>

                                  <span className={`${order.goods_price} ${order.width_10} ${order.center}`}
                                        style={{ width: '18.18%' }}>￥{child.spellPrice}</span>
                                  <span className={`${order.buy_num} ${order.width_10} ${order.center}`}
                                        style={{ width: '18.18%' }}>{child.productNum}</span>
                                </div>
                              </div>
                              <div
                                className={`${order.pay_amount} ${order.width_10} ${order.center}`}
                                style={{ width: '11.76%' }}>￥{child.orderAmount}</div>

                              <div
                                className={`${order.order_state} ${order.width_10} ${order.center}`}
                                style={{ width: '11.76%' }}>{child.orderStateValue}</div>
                              <div
