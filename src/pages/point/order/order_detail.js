...
import { Form, Radio, Modal, Select, Input, Popconfirm } from 'antd';
import {
  failTip,
  sucTip,
  getSldHorLine,
...
export default class Order_detail extends Component {
        ...
        if (res.state == 200) {
          ...
          for (let item in order_info) {
            if (order_info[item].name == 'orderTypeValue') {
              order_info[item].text = (order_detail.isVirtualGoods == 2 ? `${sldComLanguage('虚拟订单')}` : `${sldComLanguage('普通订单')}`);
            } else {
  ...
  render() {
    const {
      ...
      modalVisible, operateData, deliverModal, expressList, exterFaceSheet, showPrintExterFaceSheetBtn, isAllowApplyExpressNum, reserve_info,
    } = this.state;
              ...
              <div className={order.btnsty}>
                {order_detail.isVirtualGoods == 2
                  ? <Popconfirm
                    ...
                    <div className={order.deliver_btn} style={{ background: 'rgba(255, 113, 30, .9)' }}>
                      <a style={{ color: '#fff' }}
                         href="javascript:void(0)">{sldComLanguage('发货')}
                      </a>
                    </div>
                  </Popconfirm>
                  : <div onClick={() => this.agreeReturn('deliver')} style={{ background: 'rgba(255, 113, 30, .9)' }}
        ...
        >
          <Form layout="horizontal">
            ...
            <FormItem
              ...
              {...formItemLayoutModal}
            >
              <span style={{ wordBreak: 'break-all' }}>{order_detail.receiverAddress}</span>
            </FormItem>
            <FormItem
