...
export default class DiyLists extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      addData: [{
        type: 'input',
        label: `${sldComLanguage('装修页面名称')}`,
        name: 'decoName',
        placeholder: `${sldComLanguage('请输入装修页面名称')}`,
        initialValue: '',
        maxLength: 8,
        ...
        rules: [{
          required: true,
          whitespace: true,
          message: `${sldComLanguage('请输入装修页面名称')}`,
        }],
      },
      ],//modal框的数据
      columns: [
        ...
        },
        {
          ...
          render: (text, record) => (
            <Fragment>
              ...
              <span className={global.splitLine}></span>
              {sldtbaleOpeBtnText(`${sldComLanguage('复制')}`, () => this.operateDiyPage(record.decoId, 'copy'))}
            </Fragment>
          ),
        },
  ...
  addData = () => {
    ...
    this.setState({ modalVisible: true, type: 'add', title: `${sldComLanguage('新增装修页面')}`, addData: addData });
  };
