...
export default class Add_template extends Component {
	constructor(props) {
		...
		this.state = {
			data: sldEmptyHandle2(localStorage.getItem('pc_diy_tpl')) != '' ? JSON.parse(localStorage.getItem('pc_diy_tpl').replace(/&quot;/g,"\"")) : '',
		};
	}
	...
	render() {
		...
		let data_json = JSON.parse(data.json.replace(/&quot;/g,"\""));
		return (
            ...
            <Adv_20 back_route={data.back_route} tpl_info={data_json} save_tpl_data={this.save_tpl_data}/>
            }
            {data_json.type == 'adv_21' &&
            <Adv_21 back_route={data.back_route} tpl_info={data_json} save_tpl_data={this.save_tpl_data}/>
            }
