...
@connect(({ mdecorate, pc_home, project }) => ({
  mdecorate,
  pc_home,
  project,
}))
...
export default class Edit_m_diy_page extends Component {
  constructor(props) {
    super(props);
    sthis = this;
    ...
    this.state = {
      ...
      show_top_cat_nav: props.location.query.type != undefined && props.location.query.type == 'home' ? true : false,//是否显示顶部分类导航，默认显示
  ...
  onDragEnd = result => {
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      if (result.draggableId == 'lunbo') {
        ...
        //拖动的类型为图片组合
        con.data = [];
        for (let i in sld_com_empty_arrar_2) {
          con.data.push({
            img: '',
            img_path: '',
            title: '',
        ...
        //拖动的类型为推荐商品
        con.isshow_sales = 0;//是否展示销量
        con.cart_icon_type = 1;//购物车图标样式
        con.show_style = 'small';//展示类型：big 大图 small 一行两个 list 列表 一行一个
  ...
  get_diy_page_detial = (id) => {
    const { dispatch } = this.props;
    let { selected, query, center_fixed_data, show_top_cat_nav } = this.state;
    dispatch({
      ...
      callback: (res) => {
        if (res.state == 200) {
          ...
          this.setState({ selected: selected, center_fixed_data, show_top_cat_nav });
        } else {
          failTip(res.msg);
        }
      },
    ...
    return (
      <div style={{ width: '100%', height: '100%' }}>
        <div className={global.flex_com_space_between} style={{ margin: 10 }}>
          {sldLlineRtextAddGoods('#4877F7', query.name)}
          {sldIconBtnBg(() => this.props.history.goBack(), 'ziyuan24', `${sldComLanguage('返回上级页面')}`, '#fff', 7, 0, 14, 14, 3)}
        </div>
        {getSldHorLine(1)}
        <div className={global.flex_row_between_center} style={{ height: '100%' }}>
          <DragDropContext onDragEnd={this.onDragEnd}>
                        ...
                        {(provided, snapshot) => {
                          ...
                          >
                            <div className={`${mdiy.image_wrap} ${global.flex_row_center_center}`}>
                              <ALibbSvg fill={'#FF711E'} width={40} height={40} type={item.icon}/>
                            </div>
                            <span>{item.name}</span>
                          </div>;
                        }}
            ...
            <div style={{ flex: 1, height: '100%' }}>
              <div className={`${mdiy.center_wrap} ${global.flex_row_center_center}`} style={{height:"100%"}}>
              <Scrollbars
          ...
          <div className={mdiy.right_wrap}
            ...
            <Fragment>
              <div className={`${global.flex_com_row_start_center} ${mdiy.r_title}`}>
                <ALibbSvg fill={'#FF711E'} width={22} height={22} type={select_data.admin_icon}/>
            ...
            <Fragment>
              <div className={`${global.flex_com_row_start_center} ${mdiy.r_title}`}>
                <ALibbSvg fill={'#FF711E'} width={22} height={22} type={'ziyuan16'}/>
          ...
          <div className={mdiy.m_diy_bottom_wrap} style={{ position: 'fixed' }}>
            <div onClick={() => this.props.history.goBack()} className={global.add_goods_bottom_btn}>
              {sldComLanguage('返回')}
            </div>
            <div onClick={() => this.saveMDiyData()}
