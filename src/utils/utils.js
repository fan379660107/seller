...
async function requestLogout() {
  ...
  await fetch(apiUrl + 'v3/sellerLogin/oauth/logout', {
    credentials: 'include',
    headers: {
...
export function sldIconBtnBg(callback, svg, text, svgColor, wrapML, wrapMR, svgW = 15, svgH = 15, textML) {
  return <div style={{ marginLeft: wrapML, marginRight: wrapMR, backgroundColor: '#FF711E' }}
 ...
 * 上传图片限制，限制的大小是服务器的配置
 */
export function sldBeforeMoreUpload(allowNum,file, fileList, limit = uploadLimit) {
if(fileList.length > allowNum){
    failTip(`本次上传的图片不可超过${allowNum}张`);
    return false;
  }
  if (file.size != undefined && file.size > 1024 * 1024 * limit) {
    failTip(`上传文件过大，请上传小于${limit}M的图片`);
...
export async function sldCommonService(params, method, url, data_type = '') {
  ...
  //不需要token的接口
  let speial_request = [
    'v3/sellerLogin/oauth/token',//登录
    'v3/sellerLogin/oauth/logout',//退出登录
...
async function refreshToken() {
  ...
  let result = await fetch(apiUrl + 'v3/sellerLogin/oauth/token', {
    credentials: 'include',
    headers: {
      Authorization: 'Basic VVcxS2FsVXlWbk5pUjFaNTpVMjFHTWxsVlFrUmlNMEkxVlcxc2JtRklVa0ZWTW5oMldrYzVkUT09',
    },
...
export function getSettleData(key) {
  ...
  try {
    return JSON.parse(tmp_data.replace(/&quot;/g,"\""));
  } catch (e) {
    return tmp_data;
  }
}
